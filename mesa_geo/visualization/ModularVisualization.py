...
class SocketHandler(tornado.websocket.WebSocketHandler):
    ...
    @property
    def viz_state_message(self, co2_check=True):
        return {
            "type": "viz_state",
            "data": self.application.render_model(co2check=True)
        }
    ...
    def on_message(self, message):
        ...
        if msg["type"] == "get_step":
            ...
            if not self.application.model.running:
                ...
                self.write_message({
                    "type": "viz_state",
                    "data": self.application.render_model(co2_render)
                })
            ...
            self.write_message({
                ...
                "start_tick": self.application.start_tick
            })
...
class ModularServer(tornado.web.Application):
    ...
    def render_model(self, co2check):
        """ Turn the current state of the model into a dictionary of
        visualizations

        """
        visualization_state = []
        print('self.visualization_elements')
        print(self.visualization_elements)
        print('co2check set? %s' %co2check)
        if co2check == False:
            self.visualization_elements.pop()
        for element in self.visualization_elements:
            type = element.type
            print('type in visualization_element = %s' %type)
            if type == "co2":
                if co2check == True:
               ...
               # if data_collector_name == "co2_cost":
               #     if co2check == True:
               #         element_state = element.render(self.model)
               #         visualization_state.append(element_state)
               #         {"type": "get_step", "step": 8, "co2check": "on"}
